...
const f = "float PI = 3.141592653589793238;",
    ...
    R = e => {
        let n = G;
        Array.isArray(e) && e.length && (n = {}, Object.keys(G).forEach((t => {
            e.includes(t) && (n[t] = G[t])
        })));
        const t = Math.floor(Math.random() * Object.keys(n).length);
        return n[Object.keys(n)[t]]
    };
class K {
    constructor(e) {
        const n = "random" === e.shader || Array.isArray(e.shader) ? R(e.shader) : G[e.shader];
        this.shader = n, this.displacement = e.displacementMap || d, this.scene = new a, this.swiper = e.swiper, this.vertex = n.vertex || C, this.fragment = n.fragment, this.uniforms = n.uniforms || {}, this.renderer = new l({
            dpr: 2,
            webgl: 2
        }), this.gl = this.renderer.gl, this.width = window.innerWidth, this.height = window.innerHeight, this.renderer.setSize(this.width, this.height), this.gl.clearColor(1, 1, 1, 1), this.opts = e, this.container = this.swiper.el, this.images = [], this.displacementTexture = null, this.container.querySelectorAll(".swiper-gl-image").forEach((e => {
            this.images.push(e.src)
        })), this.width = this.swiper.width, this.height = this.swiper.height, this.container.prepend(this.gl.canvas), this.camera = new c(this.gl, {
            fov: 45
    ...
    animateUniform(e, n, t) {
        ...
        const s = n > e.value ? "next" : "prev",
            ...
            l = () => {
                ...
                let m = o + c * (n - o);
                if (a(m, n) && (m = n), e.value = m, a(m, n)) return cancelAnimationFrame(this.animateUniformFrame), void(t && t());
    ...
    init(e) {
        const n = [],
            t = this;
        this.images.forEach(((e, o) => {
            const i = new Promise((n => {
                const i = new Image;
                i.crossOrigin = "anonymous";
                const r = new m(this.gl);
                i.onload = () => {
                    r.image = i, t.textures[o] = r, n()
                }, i.src = e
            }));
            n.push(i)
        })), n.push(new Promise((e => {
            const n = new Image;
    ...
    resize() {
        ...
        this.width = e, this.height = n, this.renderer.setSize(e, n);
        ...
        let i, r;
        n / e > o ? (i = e / n * o, r = 1) : (i = 1, r = n / e / o), this.material.uniforms.resolution.value.x = e, this.material.uniforms.resolution.value.y = n, this.material.uniforms.resolution.value.z = i, this.material.uniforms.resolution.value.w = r, this.shader.vertex && this.vertexMaterial && (this.vertexMaterial.uniforms.resolution.value.x = e, this.vertexMaterial.uniforms.resolution.value.y = n, this.vertexMaterial.uniforms.resolution.value.z = i, this.vertexMaterial.uniforms.resolution.value.w = r), this.shader.vertex ? (this.nextMesh.scale.set(this.camera.aspect / 2, .5, .5), this.currentMesh.scale.set(this.camera.aspect / 2, .5, .5)) : (this.plane.scale.x = this.camera.aspect, this.plane.scale.y = 1)
    }
    createMaterial() {
        return new h(this.gl, ((e, r) => {
            for (var s in r || (r = {})) t.call(r, s) && i(e, s, r[s]);
            if (n)
                for (var s of n(r)) o.call(r, s) && i(e, s, r[s]);
            return e
        })({
            extensions: {
        ...
        if (this.scene.children.forEach((e => {
            ...
            const e = function(e, n, t) {
                    ...
                    for (let s = 0; s < a; s += 1) {
                        d = s * r, u = s % 2 == 1 ? -i / 2 : 0;
                        for (let e = 0; e <= o; e += 1) {
                            const o = Math.sign(e * i + u - 1);
                            c.push(e * i + u - 1, d - 1, 0), x.push((e * i + u) / 2, d / 2), c.push(e * i + i / 2 + u - 1, r + d - 1, 0), x.push((e * i + i / 2 + u) / 2, (r + d) / 2), c.push(e * i - i / 2 + u - 1, r + d - 1, 0), x.push((e * i - i / 2 + u) / 2, (r + d) / 2);
                            let f = y(e / a, s / a) + Math.random();
                            const U = w(d / 2 + 2 * f / n);
                            let E = Math.random();
                            l.push(U, w(U + .1 * t), w(U + .1 * t)), g.push(E, E, E);
                            const F = [e * i + u - 1, d - 1, 0];
                            m.push(...F, ...F, ...F);
                            const G = [2 * o * M(-.3, .3), -2 * M(-.3, .3) * 1.5, -A(.5)],
                                C = [2 * o * M(.3, .6), -2 * M(.3, .6) * 1.5, -A(.5)];
                            h.push(...G, ...G, ...G), p.push(...C, ...C, ...C), v.push(0, 0, 1, 0, 1, 0, 1, 0, 0), c.push(e * i + u - 1, d - 1, 0), x.push((e * i + u) / 2, d / 2), c.push(e * i + i + u - 1, d - 1, 0), x.push((e * i + i + u) / 2, d / 2), c.push(e * i + i / 2 + u - 1, r + d - 1, 0), x.push((e * i + i / 2 + u) / 2, (r + d) / 2), f = y((e + 1) / a, s / a) + Math.random();
                            const R = w(d / 2 + 2 * f / n);
                            E = Math.random(), l.push(R, R, w(R + .1 * t)), g.push(E, E, E);
                            const K = [e * i + u - 1, d - 1, 0];
                            h.push(...G, ...G, ...G), p.push(...C, ...C, ...C), m.push(...K, ...K, ...K), v.push(0, 0, 1, 0, 1, 0, 1, 0, 0)
                        }
                    }
                    const f = new s(e);
                ...
                n = this.textures[1];
            this.vertexMaterial = this.createMaterial(), this.vertexMaterial.uniforms.texture1.value = n, this.currentMesh = new g(this.gl, {
                geometry: e,
    ...
    replaceShader(e) {
        let n, t;
        this.shader.vertex ? (n = this.material.uniforms.texture1.value, t = this.vertexMaterial.uniforms.texture1.value) : (n = this.material.uniforms.texture1.value, t = this.material.uniforms.texture2.value);
        const o = "random" === e || Array.isArray(e) ? R(e) : G[e],
            {
                fragment: i,
                uniforms: r,
                vertex: s
    ...
    replaceRandomShader() {
        const e = R(this.opts.shader),
            {
                fragment: n,
                uniforms: t,
                vertex: o
            } = e;
        this.shader = e, this.fragment = n || "", this.uniforms = t || {}, this.vertex = o || C, this.addObjects(), this.resize()
    }
    setProgress(e, n, t, o) {
        if (this.destroyed) return;
...
const Y = new u(".swiper", {
        modules: [v, function({
            ...
            n("beforeInit", (() => {
                ...
                Object.assign(e.params, n), Object.assign(e.originalParams, n)
            })), n("init", (() => {
                "gl" !== e.params.effect || o || e.gl || i()
            })), n("resize", (() => {
                "gl" !== e.params.effect || o || e.gl.resize()
            })), n("setTranslate", (() => {
                if ("gl" !== e.params.effect || o) return;
                let n, t, a;
                e.gl || i(), e.slides.forEach(((o, i) => {
                    const r = o.progress;
                    (r > 0 && r < 1 || 0 === r && e.progress < s) && (n = i, t = i + 1, a = r), (r < 0 && r > -1 || 0 === r && e.progress > s) && (n = i - 1, t = i, a = 1 + r)
                })), s = e.progress || 0, void 0 === n && void 0 === t || e.gl.setProgress(n, t, a, r)
            })), n("setTransition", ((n, t) => {
                "gl" !== e.params.effect || o || (r = t > 0 && !e.params.cssMode)
            })), n("destroy", (() => {
                "gl" !== e.params.effect || o || e.gl && (e.gl.destroy(), e.gl = null)
            }))
        }],
        speed: 1e3,
        effect: "gl",
...
document.querySelector(".demo-shader-selector").addEventListener("click", (() => {
    ...
    var n;
    "SPAN" === e.target.nodeName && (n = e.target.textContent.trim(), document.querySelector(".demo-shader-current").textContent = n, Y.gl.replaceShader(n), T.style.display = "none")
